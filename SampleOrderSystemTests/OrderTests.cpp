...
//
//       // 승인 처리: Controller가 미리 계산해 넘겨주는 availableStock(가용 재고, PLAN.md 옵션 B 참고
//       //   - sample.Stock()에서 같은 시료의 CONFIRMED·미출고 주문 수량 합계를 뺀 값이며, 같은 시료에
//       //     이미 활성 PRODUCING job이 있으면 Controller가 0을 넘김)과 주문 수량을 비교해 CONFIRMED
//       //   또는 PRODUCING으로 전이한다(availableStock >= 수량이면 CONFIRMED, 아니면 PRODUCING).
//       //   RESERVED 상태가 아니면 std::logic_error.
//       // 2026-07-15 변경(Red, PLAN.md 옵션 B): 시그니처를 Approve(const Sample&)에서
//       //   Approve(int availableStock)로 변경한다. Order/Sample은 여전히 OrderRepository나
//       //   ProductionLine을 참조하지 않으며, availableStock 계산(CONFIRMED 합계 차감, 활성 job 여부
//       //   반영)은 Controller(MenuController)가 MonitoringService::SumConfirmedQuantity와
//       //   ProductionLine::CurrentJob을 조합해 수행한 뒤 값으로 넘긴다.
...
TEST_CASE("허용되지 않는 상태 전이는 거부된다", "[Order][invalid-transition]")
{
    SECTION("REJECTED 상태에서 재승인 시도는 거부된다")
    {
        ...
        REQUIRE_THROWS_AS(order.Approve(sample.Stock()), std::logic_error);
    }
    ...
    SECTION("PRODUCING 상태에서 출고 처리를 시도하면 거부된다(CONFIRMED 아님)")
    {
        ...
        order.Approve(sample.Stock()); // 재고 0 < 10 -> PRODUCING
