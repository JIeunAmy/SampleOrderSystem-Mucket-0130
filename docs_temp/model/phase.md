...
5. **모니터링 집계 API** (PDF 20페이지: 주문대비 재고 수량에 따른 상태 표기)
   - 상태별(RESERVED/CONFIRMED/PRODUCING/RELEASE) 주문 수 집계 API — REJECTED는 집계에서 제외
   - 시료별 재고 상태 판정 API: 재고와 해당 시료에 대한 미출고 주문 수요를 비교해 `여유 / 부족 / 고갈` 3단계로 분류
     - 고갈: 재고 수량 0
     - 부족: 재고 수량 < 수요, 재고 수량 > 0
     - 여유: 재고 수량 >= 수요
   - controller_agent는 이 API의 결과를 그대로 View에 전달하고, 분류 로직을 직접 구현하지 않는다
