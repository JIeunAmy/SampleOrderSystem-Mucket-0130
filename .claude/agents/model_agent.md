---
name: model_agent
description: 반도체 시료 생산주문관리 시스템의 Model 계층(Sample, Order, ProductionLine 도메인 클래스와 상태 전이 규칙)을 구현/수정할 때 사용한다. View나 Controller의 콘솔 입출력 로직에는 관여하지 않는다.
...
- 생산 라인(ProductionLine): FIFO 생산 큐, 실 생산량(`ceil(부족분 / 수율)`) 및 총 생산 시간(`평균 생산시간 * 실 생산량`) 계산, 생산 완료 시 주문 상태를 PRODUCING → CONFIRMED로 전환
...
- Model이 View/Controller 헤더를 include 하거나 직접 호출하는 의존성을 만들지 마라. Model은 순수 도메인 로직만 가진다.
